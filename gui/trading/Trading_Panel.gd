...
var itemslot_template = preload("res://gui/game_menu/trading/Itemslot_Template.tscn")
...
var exchange_rate_buy : float = 2.0
var exchange_rate_sell : float = 0.5

var last_clicked_time : float
var last_clicked_item : Node
var compared_item : Node

export(float) var doubleclick_threshhold = 0.5
...
func _ready():
	...
	game.globals.clear_interaction_events()
	...
	player_name_label.text = game.characters.get_display_name("player")
...
func _on_confirm_button_pressed():
	...
	for item in exchange_items:
		...
		if item.item_owner == "trader":
			...
			### local interaction flags for trader reaction dialog
			game.globals.add_interaction("bought_item")
			...
			### local interaction flags for trader reaction dialog
			game.globals.add_interaction("sold_item")
	...
	### local interaction flags for trader reaction dialog
	if cost_total > 0:
		game.globals.add_interaction("gained_money")
	elif cost_total < 0:
		game.globals.add_interaction("spend_money")	
	elif cost_total == 0:
		game.globals.add_interaction("traded_equal")
